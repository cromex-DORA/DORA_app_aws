...
class DictDfInfoShp(dict):
    ...
    def import_info_MO(self):
        ...
        df_info_MO['CODE_DEP'] = df_info_MO['CODE_DEP'].fillna("0").astype('int64').astype(str)
        ...
        self['df_info_MO'].dict_CODE_MO_list_CODE_DEP = dict(zip(df_info_MO['CODE_MO'].to_list(),df_info_MO['CODE_DEP'].apply(lambda x:x.split(","))))
        return self
    ...
    def actualisation_dict_dict_info_REF(couche_REF,REF,dict_dict_info_REF):
        if REF =="PPG":
            df_info_a_ajouter = couche_REF[["CODE_"+REF,"NOM_init",'CODE_MO','NOM_MO']]
            dict_dict_info_REF['df_info_PPG'] = pd.concat([dict_dict_info_REF['df_info_PPG'],df_info_a_ajouter])
            dict_dict_info_REF = DictDfInfoShp.maj_fichier_DictDfInfoShp(dict_dict_info_REF,[REF])
        if REF == 'MO':
            df_info_a_ajouter = couche_REF[["CODE_"+REF,"NOM_init","CODE_DEP"]]
            dict_dict_info_REF['df_info_MO'] = pd.concat([dict_dict_info_REF['df_info_MO'],df_info_a_ajouter])
            dict_dict_info_REF = DictDfInfoShp.maj_fichier_DictDfInfoShp(dict_dict_info_REF,[REF])
    ...
    def mise_a_jour_df_info_GEMAPI(dict_dict_info_REF,BDD_SANDRE,BDD_SIRET,gdf_MO):
        ...
        df_info_MO.to_csv("G:/travail/carto/couches de bases/syndicats GEMAPI/fichier_info_MO_gemapi.csv",index=False)
        gdf_MO = MAJ_fichier_shp_MO(df_info_MO,gdf_MO)
        #gdf_MO.to_file("G:/travail/carto/couches de bases/syndicats GEMAPI/MO_gemapi_NA.shp")
        return df_info_MO
