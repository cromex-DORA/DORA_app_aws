...
chemin_dossier = "G:/travail/carto/projets basiques/PAOT global 5.0/"
...
#################################################################################################################
    ...
    def creation_dossiers_projet(self,liste_echelle_REF_projet):
        for nom_couche_REF in liste_echelle_REF_projet:
            if not os.path.exists("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_custom_tempo'):
                os.makedirs("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_custom_tempo')
            if not os.path.exists("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_geom'):
                os.makedirs("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_geom')
            if not os.path.exists("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_bloc'):
                os.makedirs("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_bloc')
            if not os.path.exists("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_boite'):
                os.makedirs("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_boite')
            if not os.path.exists("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_fleche'):
                os.makedirs("G:/travail/carto/projets basiques/PAOT global 5.0/" + self.nom_dossier_maitre + '/couche_fleche')
...
########################################################################################################
    ...
    def creation_dict_couche_geom(self,liste_echelle_REF_projet):
        ...
        for echelle_carto_REF in self.liste_echelle_REF_projet:
            if echelle_carto_REF == 'ME':
                perimetre_ME= gpd.read_file("G:/travail/carto/projets basiques/PAOT global 5.0/couche geom/ME/BV ME AG 2021.shp")
    ...
    def export_bloc(self):
        ...
        for nom_entite_custom,contenu_custom in self.items():
            for type_boite,contenu_boite in contenu_custom.items():
                '''df_faits = pd.read_csv("G:/travail/carto/projets basiques/PAOT global 5.0/test/repartition_df_faits.csv")
                ...
                contenu_boite.df_contour = contenu_boite.df_contour.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/"+ id_projet +"/couche_boite/boite_complete_" + id_projet + "_" + contenu_boite.orientation + ".shp",engine="fiona", encoding='utf-8')
        for nom_entite_custom,contenu_custom in self.items():
            for type_boite,contenu_boite in contenu_custom.items():
                for type_bloc,contenu_bloc in contenu_boite.items():
                    if hasattr(contenu_bloc,"df_indiv"):
                        contenu_bloc.df_indiv = contenu_bloc.df_indiv.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/"+ id_projet +"/couche_bloc/" + contenu_bloc.type + '_' + contenu_boite.orientation + ".shp",engine="fiona", encoding='utf-8') 
                    if not hasattr(contenu_bloc,"df_indiv"):
                        contenu_bloc.df = contenu_bloc.df.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/"+ id_projet +"/couche_bloc/" + contenu_bloc.type + '_' + contenu_boite.orientation + ".shp",engine="fiona", encoding='utf-8') 
        return self    
    ...
    def export_atlas(self):
        ...
        self.gdf_custom.to_file('G:/travail/carto/projets basiques/PAOT global 5.0/' + id_projet + '/atlas_' + id_projet + '.shp',engine="fiona")
        return self
    

    def export_fond_carte(self):
        id_projet = self.type_donnees + '_' + self.thematique + '_' + self.public_cible
        self.gdf_fond_carte.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/" + id_projet + "/couche_fond_carte/fond_carte_" + id_projet + ".shp",engine="fiona")
        return self    
    ############################################################################################################################
    #Osmose
    ############################################################################################################################
    ...
    def export_fichier_excel_osmose_conversion_dora(self):
        ...
        for entite_custom,contenu_custom in self.items():
            workbook.save("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/" + contenu_custom.echelle_REF + "/Tableau_suivi_osmose_" + entite_custom + ".xlsx")        
            excel_modif = load_workbook("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/" + contenu_custom.echelle_REF + "/Tableau_suivi_osmose_" + entite_custom + ".xlsx", read_only=False, keep_vba=False)
            ...
            excel_modif.save("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/" + contenu_custom.echelle_REF + "/Tableau_suivi_osmose_" + entite_custom + ".xlsx")

    def export_fichier_excel_osmose_maj(self):
        workbook = load_workbook("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/TABLEAU_MAJ_OSMOSE_VIERGE.xlsx", read_only=False)
        for type_dict_donnees,dict_donnees in self.items():
            for entite_custom,contenu_custom in dict_donnees.items():
                workbook.save("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MAJ Osmose/tableau_propre_MAJ/Tableau_suivi_MIA_MO_vierge_" + entite_custom + ".xlsx")        
                excel_modif = load_workbook("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MAJ Osmose/tableau_propre_MAJ/Tableau_suivi_MIA_MO_vierge_" + entite_custom + ".xlsx", read_only=False, keep_vba=False)
                excel_modif = tableau_excel.ajout_onglet_actions(excel_modif,contenu_custom.df)
                excel_modif = tableau_excel.ajout_onglet_attributs(excel_modif,contenu_custom.df_attributs)
                excel_modif.save("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MAJ Osmose/tableau_propre_MAJ/Tableau_suivi_MIA_MO_vierge_" + entite_custom + ".xlsx")
                if hasattr(contenu_custom,"df_actions_a_recreer"):
                    excel_modif = load_workbook("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/TABLEAU_CREATION_OSMOSE_VIERGE.xlsx", read_only=False, keep_vba=False)
                    ...
                    excel_modif.save("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MAJ Osmose/tableau_propre_MAJ/Tableau_suivi_MIA_MO_vierge_" + entite_custom + "_creer_actions" + ".xlsx")
    ...
    def export_log_df_erreur(self):
        for entite_custom,contenu_custom in self.items():
            ...
            if ENVIRONMENT=="developpement":
                contenu_custom.df_log_erreur.to_csv("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/" + contenu_custom.echelle_df +  "/log/log "+ entite_custom + ".csv",index=False)
    ...
    def export_tableau_excel_complet(self,dict_df_donnees):
        for entite_custom,contenu_custom in self.items():
            ...
            if ENVIRONMENT=="developpement":
                fichier_excel.save("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/" + contenu_custom.echelle_REF + "/Tableaux_final/Tableau_final_" + contenu_custom.NOM_custom + ".xlsx")
