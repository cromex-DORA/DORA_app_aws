...
class DictBoiteComplete(dict):
    ...
    def creation_contour_boite_complete(self,type_boite_placement):
        ...
        for nom_boite_maitre,dict_boite_complete in self.items():
            if dict_boite_complete.orientation in type_boite_placement:
                ...
                #dict_boite_complete.df_contour.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/test/essai_contour.shp")
        return self
    ##########################################################################################
    ...
    def creation_point_unique_par_REF(self,df_info_custom):
        ...
        #self.df.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/test/essai_points.shp")        
        return self
    ...
    def actualisation_haut_bas(self,type_boite_placement):
        for nom_boite_maitre,dict_boite_complete in self.items():
            if dict_boite_complete.orientation in type_boite_placement:
                #dict_boite_complete.df.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/test/point_inter_test.shp")
    ...
    def export_boite(self,un_projet):
        ...
        for nom_boite_maitre,dict_boite in dict_sans_tableau_vide.items():
            if len(dict_boite)>0:
                ...
                df_issu_dict_avec_info_placement_par_boite.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/" + un_projet.nom_dossier_maitre + '/couche_boite/boite_' + dict_nom_boite_maitre[nom_boite_maitre]['nom_boite'] + '.shp', encoding='utf-8')
            if len(dict_boite)==0:
                #On envoie un truc vide histoire d'actualiser quand méme le shp
                schema = {"geometry": "Polygon", "properties": {"id": "int"}}
                crs = "EPSG:2154"
                df = gpd.GeoDataFrame(geometry=[])
                df.to_file("G:/travail/carto/projets basiques/PAOT global 5.0/" + un_projet.nom_dossier_maitre + '/couche_boite/boite_' + dict_nom_boite_maitre[nom_boite_maitre]['nom_boite'] + '.shp', driver='ESRI Shapefile', schema=schema, crs=crs)
