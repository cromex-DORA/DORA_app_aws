...
def recuperation_liste_tableaux_info_PPG_MIA():
    ...
    try:
        liste_xlsm_info_PPG = [pd.read_excel(f,sheet_name="info_PPG") for f in glob.glob("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/MO/*.xlsm")]
        liste_nom_fichier_actions = [os.path.basename(f) for f in glob.glob("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/MO/*.xlsm")]
...
def traitement_specifique_df_info_PPG_issu_df_actions(df_info_PPG_issu_df_actions):
    df_vierge_info_PPG = pd.read_excel("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/MO/Tableau_suivi_MIA_MO_vierge.xlsm",sheet_name="info PPG")
...
def recuperer_tableau_OSMOSE():
    dict_df_MAJ = {}
    chemin_fichier_tableau = "G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MAJ Osmose"
...
def recuperer_BDD_Osmose():
    dict_df_MAJ = {}
    chemin_fichier_tableau = "G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/BDD_Osmose"
    liste_df_actions_originaux = [pd.read_excel(f,sheet_name="Actions") for f in glob.glob(chemin_fichier_tableau + "/*.xlsx")]
    liste_df_attributs_originaux = [pd.read_excel(f,sheet_name="+Attributs") for f in glob.glob(chemin_fichier_tableau + "/*.xlsx")]
...
def recuperation_df_conversion_actions_DORA_vers_CODE_Osmose():
    df_conversion_actions_DORA_vers_CODE_Osmose = pd.read_excel("G:/travail/carto/projets basiques/PAOT global 5.0/Tableau actions/MIA/VF5_Tableaux_suivi_PPG.xlsm",sheet_name="AIDE Liste des actions")
...
'''def recuperation_tableaux_pressions(GRAND_BV=str(None)):
    ...
    df_pression_ME_LB_pour_icone = pd.read_csv("G:/travail/carto/projets basiques/PAOT global 5.0/Me avec pression/Pression ME LB 2022.csv")
...
def import_df_liste_ouvrage_prioritaire_PARCE():
    df_ouvrage_prio_PARCE= pd.read_csv("G:/travail/python/DORA/moulinette/fichiers moulinette import Osmose/Ouvrage_prioritaire_apaise_AG.csv")
...
#####################################################################################################
def import_BDD_DORA():
    BDD_DORA = pd.read_csv("G:/travail/carto/projets basiques/PAOT global 5.0/BDD_DORA.csv")
    return BDD_DORA