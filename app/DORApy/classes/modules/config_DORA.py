...
def import_dict_config_actions_MIA_DORA():
    ...
    data = get_data(io.BytesIO(obj_df_config_DORA))    
    data = data[list(data.keys())[0]]

    # Convertir les premières lignes en MultiIndex pour les colonnes
    df_config_DORA = pd.DataFrame(data)
    df_config_DORA.columns = pd.MultiIndex.from_arrays([df_config_DORA.iloc[0], df_config_DORA.iloc[1]])   
    df_config_DORA = df_config_DORA.drop([0, 1]).reset_index(drop=True)
    '''df_config_DORA.columns = pd.MultiIndex.from_tuples(
        [{('commun', 'Type_action'): ('commun', 'NOM_TYPE_ACTION_DORA')}.get(x, x) for x in df_config_DORA.columns]
    )

    df = pd.DataFrame(data[1:], columns=data[0])
    df_config_DORA = pd.read_excel(filename, engine="odf", header=[0, 1])'''

    df_config_DORA.columns = df_config_DORA.columns.set_levels(
        df_config_DORA.columns.levels[1].str.replace('Type_action', 'NOM_TYPE_ACTION_DORA'), level=1
    )
    df_config_DORA = df_config_DORA.rename(columns=lambda x: x.strip())
    #Elu
    df_DORA_actions_MIA_elu = df_config_DORA.iloc[:, (df_config_DORA.columns.get_level_values(1)=="NOM_TYPE_ACTION_DORA")|(df_config_DORA.columns.get_level_values(0)=='Elu')]
    col_df_elu = [b for (a,b) in list(df_DORA_actions_MIA_elu)]
    df_DORA_actions_MIA_elu.columns = col_df_elu
    df_DORA_actions_MIA_elu.loc[~df_DORA_actions_MIA_elu["NOM_TYPE_ACTION_DORA"].isnull()]

    #Tech
    df_DORA_actions_MIA_tech = df_config_DORA.iloc[:, (df_config_DORA.columns.get_level_values(1)=="NOM_TYPE_ACTION_DORA")|(df_config_DORA.columns.get_level_values(0)=='Tech')]
    col_df_tech = [b for (a,b) in list(df_DORA_actions_MIA_tech)]
    df_DORA_actions_MIA_tech.columns = col_df_tech
    df_DORA_actions_MIA_tech.loc[~df_DORA_actions_MIA_tech["NOM_TYPE_ACTION_DORA"].isnull()]

    #Convertisseur Osmose
    df_DORA_actions_MIA_conv_Osmose = df_config_DORA.iloc[:, (df_config_DORA.columns.get_level_values(0)=='commun')]
    col_df_conv_Osmose = [b for (a,b) in list(df_DORA_actions_MIA_conv_Osmose)]
    df_DORA_actions_MIA_conv_Osmose.columns = col_df_conv_Osmose
    df_DORA_actions_MIA_conv_Osmose.loc[~df_DORA_actions_MIA_conv_Osmose["NOM_TYPE_ACTION_DORA"].isnull()]
    df_DORA_actions_MIA_conv_Osmose = df_DORA_actions_MIA_conv_Osmose.rename({'Code Osmose associé':"CODE_TYPE_ACTION_OSMOSE",'Intégration possible du PAOT':'compatible_PAOT'},axis=1)
